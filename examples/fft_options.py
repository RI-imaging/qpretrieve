...
for fft_interface in interfaces_available:
    ...
    data_3d = np.repeat(data_3d, repeats=n_transforms, axis=0)
    data_3d_bg, _ = convert_data_to_3d_array_layout(data_2d_bg)
    data_3d_bg = np.repeat(data_3d_bg, repeats=n_transforms, axis=0)
...
plt.suptitle("Processing time for each FFT Filter")
plt.tight_layout()
# plt.show()
